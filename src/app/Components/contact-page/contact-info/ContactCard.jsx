...
const ContactCard = ({ info }) => {
  return (
    <>
      <div
  className="relative bg-[url(/images/carveCard.png)] rounded-[20px] px-5 py-6 xl:px-9 xl:py-11 w-full overflow-hidden"
>
  ...
  <p className="text-sm lg:text-lg border-t pt-4 lg:pt-6 xl:pt-3">{info?.contact}</p>
    ...
    <div style={{border: "10px solid white"}} className="absolute w-8 h-8 lg:w-11 lg:h-11 xl:w-15 xl:h-15 rounded-full -bottom-2 hover:bg-[#32BADE] hover:text-white flex items-center cursor-pointer justify-center -right-2">
      <ArrowUpRight/>
    </div>
      </div>
